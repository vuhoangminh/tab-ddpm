...
def train_ctabgan(
    ...
    if change_val:
        X_num_train, X_cat_train, y_train, _, _, _ = lib.read_changed_val(
            real_data_path
        )
    else:
    ...
    print(train_params)
    synthesizer = CTABGAN(
        df=X, test_ratio=0.0, **ctabgan_params, **train_params, device=device
    )
...
def sample_ctabgan(
    ...
    X_cat = (
        gen_data[cat_features].drop("y", axis=1).values if len(cat_features) else None
    )
    X_num = (
        gen_data.values[:, : X_num_train.shape[1]] if X_num_train is not None else None
    )
...
def main():
    ...
    sample_ctabgan(
        ctabgan, args.parent_dir, args.real_data_path, args.train_size, change_val=True
    )
